...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        # BEGIN Problem 2
        if exit is not None and exit.entrance is None:
            exit.entrance = self
        # END Problem 2
...
class WallAnt(Ant):
    ...
    def __init__(self, health=4):
        super().__init__(health)
        #因为最初的init时不止定义了health，如果直接self.health会在初始化时丢信息（比如位置信息）
# END Problem 6
...
class ContainerAnt(Ant):
    ...
    def store_ant(self, ant):
        ...
        if self.ant_contained is not None:
            assert False, "Ant already stored."
        elif ant.is_container:
            assert False, "Cannot contain a container."
        else:
...
# END Problem 12
    ...
    def remove_from_ant(self, place):
        pass
