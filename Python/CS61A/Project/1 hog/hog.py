...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for i in range(num_rolls):
        dice_number = dice()
        if is_sow_sad(dice_number):
            sow_sad = True
        total_dice_number += dice_number

    return total_dice_number if not sow_sad else 1
    # END PROBLEM 1
...
def hefty_hogs(player_score, opponent_score):
    ...
    opponent_score_str = str(opponent_score)
    result = player_score
    for digit in reversed(opponent_score_str):
        result = digit_fn(int(digit))(result)
    return result % 30
    # END PROBLEM 2
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if score0 > score1 and last_leader != 0:
        last_leader = 0
        return last_leader, f"Player {last_leader} takes the lead by {abs(score1 - score0)}"
    if score1 > score0 and last_leader != 1:
        last_leader = 1
        return last_leader, f"Player {last_leader} takes the lead by {abs(score1 - score0)}"
    else:
