...
def main():
    parser = argparse.ArgumentParser(description="Run TD(0) ε-greedy on a maze environment with multiple ε values")
    ...
    for eps in eps_list:
        print(f"Running TD(0) for ε={eps}...")
        agent = TD0Agent(maze, alpha=args.alpha, gamma=args.gamma, epsilon=eps)
        steps_list = []
        ...
        plt.title(f"TD(0) Learning Curve (ε={eps})")
        plt.grid(True)
        plt.tight_layout()
        filename = f"plots/td_eps_{eps}.png"
